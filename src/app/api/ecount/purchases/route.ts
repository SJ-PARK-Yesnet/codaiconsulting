...
export async function POST(request: Request) {
  try {
    ...
    if (!data || data.length === 0) {
      return NextResponse.json(
        { error: '등록할 구매 데이터가 필요합니다.' },
        { status: 400 }
      );
    }

    const apiUrl = `https://${domain}${zone}.ecount.com/OAPI/V2/Purchases/SavePurchases?SESSION_ID=${sessionId}`;

    // 요청 데이터 구성
    const requestData = {
      "PurchasesList": data.map((item: any) => ({
        "BulkDatas": item
      }))
    };

    console.log('구매 등록 API URL:', apiUrl);
    console.log('구매 등록 요청 데이터:', JSON.stringify(requestData, null, 2));
    ...
    return NextResponse.json(
      { success: false, message: '구매 API 처리 중 오류가 발생했습니다.', error: (error as Error).message },
      { status: 500 }
    );
  }
} 